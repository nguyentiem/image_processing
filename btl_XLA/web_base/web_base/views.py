...
def begin(img_path,arr_img_path,feature,distance_type):
    ...
    if distance_type=="EU":
        ...
        # print(dis)
        index = np.argsort(dis)[:3]
        path = []


        # path.append(arr_img_path[index[2]])

        # print(index)
        for i in range(len(index)):
            tem = arr_img_path[index[i]].split("images\\")[1]
            tem = "/static/images/" + tem
            print(dis[index[i]])
            path.append(tem)
        tem = img_path.split("images\\")[1]
        tem = "/static/images/" + tem
        path.append(tem)
        return path
...
def xulyanh(request):
    ...
    listFiles = request.GET.getlist('listFiles[]', None)
    listMethod = request.GET.getlist('listMethod[]', None)
    ruler = request.GET.get('ruler', None)
    # print(fileName)
    #

    # path = begin(fileName, listFiles, listMethod, ruler)
    # print(path)
    path = ['/static/images/00952.png', '/static/images/22.jpg', '/static/images/35.jpg', '/static/images/00943.png']
    print(path);
    # a = "/static/images/00944.png"
    data = {'status': 200, 'data':path }
